...
Deno.serve(async (req) => {
    ...
    bucketConfig.allowed_mime_types = ["audio/mpeg", "audio/wav", "audio/m4a", "audio/webm"];
    bucketConfig.file_size_limit = 20971520;
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for voice-recordings" ON storage.objects FOR SELECT USING (bucket_id = 'voice-recordings');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for voice-recordings" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'voice-recordings');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for voice-recordings" ON storage.objects FOR UPDATE USING (bucket_id = 'voice-recordings');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for voice-recordings" ON storage.objects FOR DELETE USING (bucket_id = 'voice-recordings');`
    ];
    ...
    return new Response(JSON.stringify({
        ...
        name: 'voice-recordings',
        public: true,
        allowed_mime_types: ["audio/mpeg", "audio/wav", "audio/m4a", "audio/webm"],
